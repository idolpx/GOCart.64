...
OPTIONS_ELEMENT *elements = (OPTIONS_ELEMENT *)(scratch_buf + (sizeof(scratch_buf)/2));

uint8_t options_dir(OPTIONS_STATE *state) {
...
uint8_t options_dir_up(OPTIONS_STATE *state, bool root) {
...
uint8_t options_prev_next_page(OPTIONS_STATE *state) {
   return handle_page_end();
}

uint8_t options_select(OPTIONS_STATE *state, uint8_t flags, uint8_t element_no) {
...
const MENU options_menu = {
   .state = &options_state,
   .dir = (uint8_t (*)(void *))options_dir,
   .dir_up = (uint8_t (*)(void *, bool))options_dir_up,
   .prev_page = (uint8_t (*)(void *))options_prev_next_page,
   .next_page = (uint8_t (*)(void *))options_prev_next_page,
   .select = (uint8_t (*)(void *, uint8_t, uint8_t))options_select
};
...
OPTIONS_ELEMENT * options_add_text_element(OPTIONS_STATE *state, options_func callback, const char *text) {
...
uint8_t options_callback(OPTIONS_STATE *state, OPTIONS_ELEMENT *element, uint8_t flags) {
...
void options_add_callback(OPTIONS_STATE *state, void (*callback)(uint8_t), const char *text, uint8_t flags) {
...
uint8_t options_prev_select(OPTIONS_STATE *state, OPTIONS_ELEMENT *element, uint8_t flags) {
...
uint8_t options_prev_dir(OPTIONS_STATE *state, OPTIONS_ELEMENT *element, uint8_t flags) {
