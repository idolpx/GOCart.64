...
OPTIONS_STATE * build_options(const char *title, const char *message) {
...
uint8_t handle_unsupported(const char *file_name) {
    ...
    return handle_unsupported_ex("Unsupported", "File is not supported or invalid", file_name);
}
...
uint8_t handle_unsaved_crt(const char *file_name, void (*handle_save)(uint8_t)) {
   OPTIONS_STATE *options = build_options("Unsaved changes", "How do you want to save the changes to CRT?");
   options_add_text_block(options, file_name);
...
uint8_t handle_upgrade_menu(const char *firmware, uint8_t element_no) {
   ...
   OPTIONS_STATE *options = build_options("Firmware Upgrade", "This will upgrade the firmware to");
